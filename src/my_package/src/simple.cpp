...
int main(int argc, char **argv) { // We start the main C++ program
  ...
  /* FIRST EXAMPLE - NO LOOP */
  // ROS_INFO("Help me Obi-Wan Kenobi, you're my only hope"); // This is the
  // same as a print in ROS ros::spinOnce(); // Calling ros::spinOnce() here is
  // not necessary for this simple program, because we are not // receiving any
  // callbacks. However, if you were to add a subscription into this
  // application, and                      // did not have ros::spinOnce() here,
  // your callbacks would never get called. So, add it for good
  //                  // measure.
  ...
  while (ros::ok())       // Endless loop until Ctrl + C
  {
    ...
    ros::spinOnce();
    loop_rate
        .sleep(); // We sleep the needed time to maintain the Rate fixed above
  }
